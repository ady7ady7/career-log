...
- ADDED PAGINATION AND FINISHED the Otodom scraper! It's very simple, we start from a counter of 1 and a set int of max_pages, and enclose the whole scraping procedure in a while loop, which works until our current page is lower than the max_pages. I chose to follow this implementation, since the code for my scraper is strictly sequential. At the end of the sequence we simply jump to the next page, increase the counter, and since the while condition is still in place, we automatically follow the process once again for the next page :)). (3 hr, including error handling below)

I've also enclosed the whole scraping code in a try-except block with finally block to handle VERY RARE(1 in 100~) cases where we encounter 404 error during scraping. It causes me to lose all the scraped ads, and this 
